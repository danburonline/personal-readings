...
- [ ] First enrichment pass -- pick 5-10 heavily-read papers and add Author nodes, Concept nodes, Covers edges, and Cites edges to seed.jsonl
...
- [ ] Append Paper node + InFolder edge to `seed.jsonl`
- [ ] Reload: `nanograph load readings.nano --data seed.jsonl --mode merge`
- [ ] Git commit
...
- [ ] Enrich `seed.jsonl`: Author nodes + WrittenBy edges
- [ ] Enrich `seed.jsonl`: Concept nodes + Covers edges
- [ ] Enrich `seed.jsonl`: Cites / Extends / Contradicts edges to other papers in collection
- [ ] Enrich `seed.jsonl`: Informs edges to relevant manuscripts (05-ocm, frontiers-consciousness-engineering, cortical-reorganisation, hybrid-mind-uploading)
- [ ] Reload: `nanograph load readings.nano --data seed.jsonl --mode merge`
- [ ] Index to Anara

### Monthly

- [ ] Audit: `nanograph describe --db readings.nano` -- check paper count matches `find . -name "*.pdf" | wc -l`
- [ ] Health: `nanograph doctor readings.nano`
